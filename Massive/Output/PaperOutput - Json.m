...
GenSection[particles_] := With[{
  re = BasisResultDict2JsonObj[particles, BasisByModel[particles, dimFrom, dimTo, "form" -> List, output -> "tex"]],
  path = FileNameJoin[{$MassiveDir, "Output",
    ToString[Abs[$currentModel[#]["spin"]] & /@ particles // Total],
    "result_" <> StringJoin @@ particles <> ".json"}] // ExpandFileName},
  Export[path, re, "JSON"]
];
